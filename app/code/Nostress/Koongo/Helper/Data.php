...
//     		return $this->getBaseUrl(\Magento\Framework\UrlInterface::URL_TYPE_MEDIA);
    ...
    public function isMagentoVersionEqualOrGreaterThan($version)
    {
		...
		foreach ($currentVersionArray as $index => $currentVersionItem)
		{
			if(!isset($versionArray[$index]))
				return null;
			else if($versionArray[$index] > $currentVersionItem)
				return false;			
		}
		return true;
    }
    ...
    */
    ...
    public function getStoreConfig($storeId = null, $configPath)
    {
    	if(!isset($storeId))
    ...
    public function getStoreConfigFlag($storeId = null, $configPath)
    {
    	if(!isset($storeId))
	...
	*/
    ...
    public function getStoresWithLocale()
    {
    	...
    	foreach ($allStores as $store)
    	{
    		...
    		else if(isset($configArray['websites'][$webId]))
    ...
    public function getStoreCurrenciesOptionArray($storeId)
    {
    	...
    	{
    		unset($codes[$index]);
    		$options = array($defaultCurrencyCode => $defaultCurrencyCode." (".__("Default Display Currency").")");
    	}
    	else
    		$options = array();
