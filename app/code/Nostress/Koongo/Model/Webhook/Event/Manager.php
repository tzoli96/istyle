...
class Manager extends \Nostress\Koongo\Model\AbstractModel
{
	...
	public function addCancelOrderEvents($order, $cancelReason = "")
	{
		...
		return $this->addWebhookEvents($topic, null, $order->getId(), $order->getStoreId(), array(\Nostress\Koongo\Model\Webhook\Event::PARAM_NOTE => $cancelReason));				
	}
	...
	public function addShipmentEvents($shipment, $track)
	{
		...
		$params = array(\Nostress\Koongo\Model\Webhook\Event::PARAM_STORE_SHIPMENT_ID => $shipment->getId(),
	...
	public function addNewProductEvents($product)
	{
		...
		$params = array(\Nostress\Koongo\Model\Webhook\Event::PARAM_STORE_PRODUCT_ID => $product->getId(),
	...
	public function addBatchProductEvents($productSkusToUpdate)
	{
        ...
        {
            $params = array(\Nostress\Koongo\Model\Webhook\Event::PARAM_STORE_PRODUCT_SKUS => $skusForEvent);
	...
	public function addBatchDeleteProductEvents($idsToDelete)
	{
        ...
        {
            $params = array(\Nostress\Koongo\Model\Webhook\Event::PARAM_STORE_PRODUCT_IDS => $idsForEvent);
            $eventIds = $this->addWebhookEvents($topic, null, null, null, $params);
            $eventIdsResult = array_merge($eventIdsResult,$eventIds);
