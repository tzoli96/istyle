...
namespace Customweb\FirstDataConnectCw\Model\Cron;
...
class TransactionCleanup
{
	...
	public function execute()
	{
		...
		try {
			$this->_logger->info('FirstDataConnectCw: Cleaned up ' . $this->delete($this->getFailedTransactions(), $maxEndtime) . ' failed transactions.');
		} catch (\Exception $e) {
			$this->_logger->error('Error in FirstDataConnectCw transaction cleanup cron: ' . $e->getMessage());
		}

		//Remove all pending transaction 6 month after last update
		try {
			$this->_logger->info('FirstDataConnectCw: Cleaned up ' . $this->delete($this->getPendingTransactions(), $maxEndtime) . ' pending transactions.');
		} catch (\Exception $e) {
			$this->_logger->error('Error in FirstDataConnectCw transaction cleanup cron: ' . $e->getMessage());
		}
		...
		try {
			$this->_logger->info('FirstDataConnectCw: Cleaned up ' . $this->delete($this->getNoStatusTransactions(), $maxEndtime) . ' transactions with no status.');
		} catch (\Exception $e) {
			$this->_logger->error('Error in FirstDataConnectCw transaction cleanup cron: ' . $e->getMessage());
		}
	}
	...
	/**
	 * @param \Customweb\FirstDataConnectCw\Model\Authorization\Transaction[] $items
	...
	/**
	 * @return \Customweb\FirstDataConnectCw\Model\ResourceModel\Authorization\Transaction\Collection
	 */
