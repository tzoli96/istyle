...
class Service implements \Customweb_Payment_ExternalCheckout_ICheckoutService
{
	...
	/**
	 * @var \Customweb\FirstDataConnectCw\Model\Authorization\OrderContextFactory
	 */
	...
	/**
	 * @param \Customweb\FirstDataConnectCw\Model\DependencyContainer $container
	 * @return \Customweb\FirstDataConnectCw\Model\ExternalCheckout\Service
	 */
	public function setContainer(\Customweb\FirstDataConnectCw\Model\DependencyContainer $container)
	{
	...
	{
		if (!($context instanceof \Customweb\FirstDataConnectCw\Model\ExternalCheckout\Context)) {
			throw new \Customweb_Core_Exception_CastException('Customweb\FirstDataConnectCw\Model\ExternalCheckout\Context');
		}
	...
	public function authenticate(\Customweb_Payment_ExternalCheckout_IContext $context, $emailAddress, $successUrl)
	{
		...
		if ($this->_checkoutHelper->isAllowedGuestCheckout($context->getQuote())
				&& $this->_scopeConfig->getValue('firstdataconnectcw/shop/external_checkout_account_creation') == 'skip_selection') {
			...
			$context->setRegisterMethod(\Customweb\FirstDataConnectCw\Model\ExternalCheckout\Context::REGISTER_METHOD_GUEST);
		...
		$context->save();
		return \Customweb_Core_Http_Response::redirect($context->getQuote()->getStore()->getUrl('firstdataconnectcw/externalCheckout/login', ['_secure' => true]));
	}
	...
	public function renderShippingMethodSelectionPane(\Customweb_Payment_ExternalCheckout_IContext $context, $errorMessages)
	{
		...
		return $this->_layoutFactory->create()->createBlock('Customweb\FirstDataConnectCw\Block\ExternalCheckout\ShippingMethods')
			->setContext($context)
	...
	public function renderReviewPane(\Customweb_Payment_ExternalCheckout_IContext $context, $renderConfirmationFormElements, $errorMessage)
	{
		...
		$layout->getUpdate()->load(['firstdataconnectcw_externalcheckout_review']);
		$layout->generateXml();
		$layout->generateElements();
		$layout->getBlock('customweb_firstdataconnectcw_externalcheckout_review')
			->setContext($context)
	...
	public function createOrder(\Customweb_Payment_ExternalCheckout_IContext $context)
	{
		...
		try {
			...
			$transaction = $this->_coreRegistry->registry('firstdataconnectcw_transaction');
