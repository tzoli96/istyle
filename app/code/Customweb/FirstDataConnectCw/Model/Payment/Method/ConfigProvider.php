...
class ConfigProvider implements \Magento\Checkout\Model\ConfigProviderInterface
{
    ...
    public function __construct(
        ...
        foreach (array_keys($paymentHelper->getPaymentMethods()) as $code) {
        	if (strpos($code, 'firstdataconnectcw_') === 0) {
    ...
    protected function getFailureMessage(AbstractMethod $method)
    {
    	if ($this->_checkoutSession->getQuote()->getPayment()->getMethod() == $method->getCode()) {
	    	$failureMessage = $this->_checkoutSession->getFirstDataConnectCwFailureMessage();
	    	if (!empty($failureMessage)) {
	    		$this->_checkoutSession->unsFirstDataConnectCwFailureMessage();
    ...
    protected function getImageUrl(AbstractMethod $method)
    {
    	try {
    		...
    		return $this->_assetRepo->getUrlWithParams('Customweb_FirstDataConnectCw/images/payment/method/' . $method->getPaymentMethodName() . '.png', $params);
    ...
    {
    	return new \Customweb\FirstDataConnectCw\Model\Renderer\CheckoutForm($method->getCode());
    }
