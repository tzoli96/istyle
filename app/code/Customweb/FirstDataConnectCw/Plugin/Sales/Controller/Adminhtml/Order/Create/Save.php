...
namespace Customweb\FirstDataConnectCw\Plugin\Sales\Controller\Adminhtml\Order\Create;
...
class Save
{
	...
	public function aroundExecute(\Magento\Sales\Controller\Adminhtml\Order\Create\Save $subject, \Closure $proceed)
	{
		...
		$this->_coreRegistry->register('firstdataconnectcwcheckout_moto', true);
		$result = $proceed();
		if ($this->_messageManager->getMessages()->getLastAddedMessage() != null
			...
			$order = $this->_coreRegistry->registry('firstdataconnectcw_checkout_last_order');
			if ($order instanceof \Magento\Sales\Model\Order) {
				if ($order->getPayment()->getMethodInstance() instanceof \Customweb\FirstDataConnectCw\Model\Payment\Method\AbstractMethod) {
					...
					return $this->_redirectResultFactory->create()->setPath('firstdataconnectcw/checkout/payment', ['order_id' => $order->getId(), 'send_confirmation' => $sendConfirmation]);
