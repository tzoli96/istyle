 ...
 * @Bean
 */
class Customweb_FirstDataConnect_BackendOperation_Adapter_CaptureAdapter extends Customweb_FirstDataConnect_AbstractAdapter implements 
	...
	public function partialCapture(Customweb_Payment_Authorization_ITransaction $transaction, $items, $close){
		...
		Customweb_FirstDataConnect_Util::setTxTypeForPaymentMethod($paymentMethod, Customweb_FirstDataConnect_IConstants::OPERATION_CAPTURE, 
				$ipgTransaction);
		...
		$service->setClientCertificate(Customweb_FirstDataConnect_Util::getCertificate($this->getConfiguration(), $transaction->isMoto()))->setClientCertificatePassphrase(
